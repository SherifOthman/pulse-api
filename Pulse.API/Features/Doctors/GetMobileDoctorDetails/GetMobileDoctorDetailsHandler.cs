        ...
        GetMobileDoctorDetailsQuery request, CancellationToken ct)
    {
        ...
        var b = await db.Businesses
            ...
            .Select(x => new
            {
                ...
                IsFavorite      = userId != null
                    && db.UserFavorite.Any(f => f.UserId == userId.Value && f.BuissnessId == x.Id),
                HasUserReviewed = userId != null
                    && x.Testimonials.Any(t => t.UserId == userId.Value),
                WorkingDays  = x.WorkingDays
                    .Select(w => new { w.Day, w.StartTime, w.EndTime })
                    .ToList(),
                PhoneNumbers = x.PhoneNumbers
                    .Select(p => new { p.Number, p.Type })
                    .ToList(),
                Branches = x.Branches
                    .Select(br => new
                    {
                        ...
                        VisitPrice      = br.Doctor!.VisitPrice,
        ...
        return new DoctorMobileDetailsResponse(
            ...
            b.WorkingDays
                ...
                .ToList(),
            b.PhoneNumbers
                .Select(p => new PhoneNumberDto(p.Number, p.Type))
                .ToList(),
            b.Branches
                .Select(br => new BranchDto(
                ...
                .ToList(),
            b.Testimonials
                .Select(t => new TestimonialDto(t.Id, t.FullName, t.ImageUrl, t.Rating, t.Text, t.CreatedAt))
                .ToList(),
            b.Services
