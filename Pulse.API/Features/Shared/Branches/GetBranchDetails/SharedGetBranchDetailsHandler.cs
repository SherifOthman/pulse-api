...
public class SharedGetBranchDetailsHandler(AppDbContext db)
    : IRequestHandler<SharedGetBranchDetailsQuery, BranchDetailsResponse?>
{
    public async Task<BranchDetailsResponse?> Handle(SharedGetBranchDetailsQuery request, CancellationToken ct)
    {
        var b = await db.Businesses
            .AsNoTracking()
            .Where(x => x.Id == request.Id && x.Type == request.Type && x.ParentBusinessId != null)
            .Select(x => new
            {
