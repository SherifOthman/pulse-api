...
public class GetMobileRadiologyDetailsHandler(AppDbContext db)
    : IRequestHandler<GetMobileRadiologyDetailsQuery, RadiologyMobileDetailsResponse?>
{
    public async Task<RadiologyMobileDetailsResponse?> Handle(
        GetMobileRadiologyDetailsQuery request, CancellationToken ct)
    {
        var userId = request.CurrentUserId;

        var b = await db.Businesses
            .AsNoTracking()
            .Where(x => x.Id == request.Id && x.Type == BusinessType.Radiology && x.ParentBusinessId == null)
            .Select(x => new
            {
