...
public class GetMobilePharmacyDetailsHandler(AppDbContext db)
    : IRequestHandler<GetMobilePharmacyDetailsQuery, PharmacyMobileDetailsResponse?>
{
    public async Task<PharmacyMobileDetailsResponse?> Handle(
        GetMobilePharmacyDetailsQuery request, CancellationToken ct)
    {
        var userId = request.CurrentUserId;

        var b = await db.Businesses
            .AsNoTracking()
            .Where(x => x.Id == request.Id && x.Type == BusinessType.Pharmacy)
            .Select(x => new
            {
        ...
        return new PharmacyMobileDetailsResponse(
